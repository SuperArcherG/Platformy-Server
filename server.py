...
def upload_file():
    if request.method == "POST":
        ...
        MissingData = (
            "data" not in request.files
            or "cover" not in request.files
            or "info" not in request.files
        )
        MissingFiles = (
            cover.filename == ""
            or data.filename == ""
        ...
        InvalidFileType = (
            not allowed_file(cover.filename)
            or not allowed_file(data.filename)
            or not allowed_file(info.filename)
        )
        InvalidLogin = (
            open(os.path.join(USERS_FOLDER, username + ".txt")).read() != password
        )
        if MissingData:
        ...
        if InvalidLogin:
            return 'err004 incorrect password "' + username + '"'  # err004
        if MissingData or MissingFiles or InvalidFileType or InvalidLogin:
            ...
            owner = open(
                os.path.join(OWNERS_FOLDER, id + ".txt"), "x"
...
def change_password():
    if request.method == "POST":
        ...
        if not found:
            return 'err000 User not found: "' + username + '"'  # err000
        ValidLogin = (
            open(os.path.join(USERS_FOLDER, username + ".txt")).read() == oldPassowrd
        )
        if not ValidLogin:
